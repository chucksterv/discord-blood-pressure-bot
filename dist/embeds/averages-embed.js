...
export function createBPAverageEmbed(data, timezone) {
    ...
    if (data.leftReadingCount > 0 && data.rightReadingCount > 0) {
        embed.addFields({
            name: '🆚 Arm Comparison',
            value: `👈 **Left:** ${data.avgLeftSystolic}/${data.avgLeftDiastolic} (${data.leftReadingCount} reading${data.leftReadingCount !== 1 ? 's' : ''})
👉 **Right:** ${data.avgRightSystolic}/${data.avgRightDiastolic} (${data.rightReadingCount} reading${data.rightReadingCount !== 1 ? 's' : ''})`,
    ...
    if (data.firstReadingAt && data.lastReadingAt) {
        ...
        let timeSpan = '';
        if (days)
            timeSpan += `${Math.floor(days)}d `;
        if (hours)
            timeSpan += `${Math.floor(hours)}h `;
        if (minutes)
            timeSpan += `${Math.floor(minutes)}m`;
        if (!timeSpan)
            timeSpan = '< 1m or only 1 reading';
        embed.addFields({
...
//# sourceMappingURL=averages-embed.js.map